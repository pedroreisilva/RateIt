...
public class AddSerie extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor> {
    ...
    private void guardar() {
        ...
        try {
            getContentResolver().insert(RateItContentProvider.ENDERECO_SERIES, serie.getContentValues());

            Toast.makeText(this, getString(R.string.serie_adicionada), Toast.LENGTH_SHORT).show();
            finish();
        } catch (Exception e) {
            Snackbar.make(
                    editTextNome,
